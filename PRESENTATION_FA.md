...
## ۱۰. تحلیل و بررسی خروجی‌های پروژه

همهٔ خروجی‌های زیر توسط اسکریپت‌های پروژه تولید می‌شوند (تبدیل، EDA، مدل‌سازی). در این بخش هر خروجی به‌زبان ساده توضیح و تحلیل می‌شود.
...
### ۱۰.۱ فایل‌های CSV و Parquet

| فایل | تولیدکننده | تحلیل و کاربرد |
|------|-------------|-----------------|
| customers_clean.csv | scripts/transform.py (تابع run_all) | جدول مشتریان بعد از پاکسازی: تاریخ ثبت‌نام نرمال‌شده، مقادیر گمشدهٔ عددی با میانه پر شده، کانال و منطقه با مد یا «unknown»؛ به‌علاوه ویژگی‌های زمانی (روز هفته، ماه، فصل) و ویژگی‌های کانال (ایمیل، اینستاگرام، وب، معرف). برای مدل ریزش و بخش‌بندی استفاده می‌شود. |
| orders_clean.csv | scripts/transform.py | سفارشات پاک‌شده با تاریخ نرمال و ویژگی‌های زمانی. مبنای تجمیع تعداد و مبلغ سفارش به‌ازای هر مشتری. |
| feedback_clean.csv | scripts/transform.py | بازخوردهای متنی بعد از پاکسازی متن، حذف stopwords، Stemming و اضافه شدن ویژگی‌های متنی (طول متن، تعداد کلمه، احساسات VADER و غیره). ورودی اصلی برای تحلیل احساسات و کلمات کلیدی. |
| single_source_of_truth.csv و single_source_of_truth.parquet | scripts/transform.py (تابع build_single_source_of_truth) | منبع واحد حقیقت: ادغام بازخورد داخلی و دیتاست احساسات عمومی در یک جدول. همهٔ تحلیل‌های EDA و بینش‌های عملی روی این فایل انجام می‌شود. فرمت Parquet برای خواندن سریع‌تر در صورت وجود. |
| customers_with_segments.csv | scripts/models.py (تابع run_segmentation) | همان مشتریان پاک‌شده به‌همراه ستون segment (برچسب خوشهٔ K-Means: ۰، ۱، ۲). برای تفکیک مشتریان به گروه‌های کم‌خرید، میان‌خرید و پرخرید و طراحی پیام بازاریابی استفاده می‌شود. |

---

### ۱۰.۲ نمودارها (پوشه data/processed/plots/)
...
| نام فایل | تحلیل و تفسیر |
|----------|----------------|
| sentiment_distribution.png | **هیستوگرام** توزیع امتیاز ترکیبی احساسات (sentiment_compound). نشان می‌دهد اکثر نظرات در چه بازه‌ای قرار دارند (مثبت، منفی، خنثی). اگر قله به سمت مقادیر منفی برود یعنی باید به نارضایتی مشتریان بیشتر رسیدگی شود. |
| word_count_distribution.png | **هیستوگرام** تعداد کلمات در هر متن. طول نظرات را نشان می‌دهد؛ نظرات خیلی کوتاه یا خیلی طولانی می‌توانند برای مدل یا نمونه‌گیری متفاوت باشند. |
| source_type_counts.png | **نمودار میله‌ای** تعداد رکوردها به تفکیک منبع (internal_feedback در برابر public_sentiment). نشان می‌دهد چند بازخورد از مشتریان مستقیم و چند تا از دیتاست عمومی احساسات داریم؛ برای تعادل داده مفید است. |
| feedback_by_month.png | **نمودار میله‌ای** تعداد بازخوردها در هر ماه. الگوی زمانی تعاملات را نشان می‌دهد؛ مثلاً در چه ماهی بیشترین بازخورد دریافت شده است. |
| trend_line.png | **نمودار خطی** روند تعداد تعاملات در طول زمان (بر اساس ستون تاریخ). برای دیدن افزایش یا کاهش فعالیت در زمان مفید است. |
| scatter_wordcount_sentiment.png | **نمودار پراکندگی** بین تعداد کلمات (محور افقی) و امتیاز احساسات (محور عمودی). رابطهٔ احتمالی بین طول نظر و مثبت/منفی بودن را نشان می‌دهد؛ نقاط پرت می‌توانند نویز یا موارد خاص باشند. |
| boxplot_sentiment_noise.png | **نمودار جعبه‌ای** امتیاز احساسات. برای شناسایی مقادیر پرت (outlier) و نویز استفاده می‌شود؛ نقاط خارج از خطوط سبیل معمولاً نیاز به بررسی دارند. |
| segment_distribution.png | **نمودار میله‌ای** تعداد مشتریان در هر خوشه (segment). توزیع بخش‌بندی K-Means را نشان می‌دهد؛ برای گزارش «چند نفر در هر گروه هستند» و اولویت‌بندی بازاریابی استفاده می‌شود. |

---

### ۱۰.۳ فایل‌های مدل (پوشه data/processed/models/)

هر فایل با پسوند
joblib
ذخیره می‌شود و شامل مدل آموزش‌دیده و در صورت نیاز شیء
scaler
است.

| فایل | تولیدکننده | تحلیل و کاربرد |
|------|-------------|-----------------|
| churn_logistic.joblib | scripts/models.py (train_churn_models) | مدل رگرسیون لجستیک برای پیش‌بینی ریزش. با بارگذاری این فایل می‌توان برای مشتریان جدید احتمال ریزش را پیش‌بینی کرد و با ضرایب مدل تفسیر کرد کدام ویژگی بیشتر اثر دارد. |
| churn_naive_bayes.joblib | scripts/models.py | مدل Naive Bayes برای همان مسئلهٔ ریزش. برای مقایسه با رگرسیون لجستیک و اطمینان از پایداری نتیجه استفاده می‌شود. |
| kmeans_segment.joblib | scripts/models.py (run_segmentation) | مدل K-Means و scaler مربوط به بخش‌بندی مشتریان. با بارگذاری می‌توان مشتریان جدید را در همان خوشه‌های قبلی قرار داد. |

---

**نحوهٔ تولید مجدد همهٔ خروجی‌ها:**  
۱) اجرای تبدیل: از ریشهٔ پروژه با پایتون فراخوانی
scripts.transform.run_all
.  
۲) اجرای EDA: فراخوانی
scripts.eda.run_eda
.  
۳) اجرای مدل‌سازی: فراخوانی
scripts.models.run_all_models
.  
ترتیب اجرا باید همین باشد چون EDA به
single_source_of_truth.csv
و مدل‌سازی به جدول‌های پاک‌شده و بخش‌بندی به خروجی مدل‌ها وابسته است.
